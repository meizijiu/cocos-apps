System.register(["cce:/internal/code-quality/cr.mjs", "cc", "../Config/fruit_config"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, isValid, loader, log, Quat, v3, fruit_config, _crd, _ax, _ay, _az, _bx, _by, _bz, Utils;
      ...
      _export("Utils", Utils = {
        ...
        */
        ...
        offsetDay: function (fromTime, toTime) {
          ...
          let toString = dayjs(toTime * 1000).format('YYYY-MM-DD'); // let toDayOfYear = dayjs(toTime * 1000).dayOfYear();
        ...
        loadResWithCorrection(url, type, completeCallback) {
          this.path2repeatTime[url] = 3;

          this._loadResWithCorrection(url, type, completeCallback);
        },

        _loadResWithCorrection(url, type, completeCallback) {
        ...
        loadResArrayWithCorrection(urls, completeCallback) {
        ...
        */
        ...
        radianFromQ1SubQ2(q1, q2) {
          ...
          // log(radian);
          let radian = Math.acos(Math.abs(Quat.dot(q1, q2))) * 2; // radian = radian * 180 / Math.PI * 2;
        ...
        //   })
        // },
        offsetNodePos(node, offset) {
          let pos = v3(node.position);
          pos.add(offset);
          node.position = pos;
        },
        ...
        randomNextFruit() {
          ...
          for (let i = 0; i < (_crd && fruit_config === void 0 ? (_reportPossibleCrUseOffruit_config({
            error: Error()
